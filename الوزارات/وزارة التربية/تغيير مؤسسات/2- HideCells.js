{
    First_Filter(event){
        ...
        let Values = [
            ...
            },
            {
                ...
                Showed_Value: [
                    "العياضية", "البعاج", "الحضر", "الكوير", "الموصل", "المحلبية", "القحطانية", "القيروان", "القيارة", "القوش", "الشمال", "شبخان", "الشورة", "تل عبطة", "وانة",
                    "الحمدانية (بغديدا)", "برطلة", "بعشيقة", "فايدة", "حمام العليل", "كنديناوة", "مخمور", "ملاقرة", "نمرود", "قراج", "ربيعة", "سنجار", "تلعفر", "تلكيف", "زيلكان",
                    "زمار", "الفاروق",
                ]
            },
            {
        ...
        if(Result){
           this.group_of_form[1].fields[18].properties[0].values = Result.Showed_Value;
        }else{
            this.group_of_form[1].fields[18].properties[0].values = [];
        }
    },

    Main_Function(Fields_Prog, Fields_ID, Condition){
    ...
    //الاسم
    First_Hide(event){
        this.Main_Function(["high"], ["89301"], event);
    },

    //شطر
    Second_Hide(event){
        this.Main_Function(["Divistion", "AfterDivision"], ["93074", "93075"], event);
    },

    //طابع الدوام
    Third_Hide(event){
        this.Main_Function(["StydyTypeOne", "StudyTypeTow"], ["93077", "93078"], event);
    },

    //جنس المؤسسة
    Fourth_Hide(event){
        this.Main_Function(["IntitGend"], ["93079"], event);
    },

    //تخفيض / رفع من والى
    Fifth_Hide(event){
        this.Main_Function(["Changefrom", "ChangeTo"], ["93080", "93081"], event);
    }
}